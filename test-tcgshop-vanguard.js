...
async function testVanguardCrawling() {
  console.log('=== TCGShop 뱅가드 카드 크롤링 테스트 시작 ===\n');
  ...
  for (const cardName of TEST_CARDS) {
    console.log(`\n🔍 검색 중: "${cardName}"`);
    try {
      const results = await crawlTCGShopVanguard(cardName, null, 'vanguard');
...
/*
  ...
  for (const cardCode of TEST_CARD_CODES) {
    console.log(`\n🔍 검색 중: "${cardCode}"`);
    try {
      const results = await crawlTCGShopVanguard(cardCode, null, 'vanguard');
...
async function testFullVanguardSearch() {
  ...
  try {
    const result = await searchAndSaveTCGShopPrices(testCardName, null, 'vanguard');
