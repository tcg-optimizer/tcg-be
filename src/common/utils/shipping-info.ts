...
// 방문수령 가능한 상점과 비용
const TAKEOUT_INFO: Record<string, number> = {
  카드킹덤: 100,
  카드냥: 100,
  카드스퀘어: 100,
  민씨지샵: 0,
...
const TAKEOUT_KEY_MAPPING: Record<string, string> = {
  cardKingdom: '카드킹덤',
  cardNyang: '카드냥',
  cardSquare: '카드스퀘어',
...
// 네이버샵의 판매자별 배송비 정보
const naverSellerShippingInfo: Record<string, any> = {
  카드킹덤: {
    shippingFee: 2800,
...
const loggedMissingShippingInfoSellers = new Set<string>();
...
function normalizeSellerName(sellerName: string): string {
...
function shouldSkipMarketplace(sellerName: string): boolean {
...
function logMissingShippingInfoOnce(sellerName: string, normalizedSellerName: string): void {
...
// 배송비 정보를 반환
function getShippingInfo(site: any): any {
