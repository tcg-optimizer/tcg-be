...
function getRandomUserAgent(includeMobile = false): string {
...
function generateRandomCookies(site: string): string {
  const commonCookies: Record<string, string> = {
    visited: 'true',
  ...
  // 사이트별 특화 쿠키
  const siteCookies: Record<string, string> = {};
...
function getSiteSpecificHeaders(
  site: string,
  additionalHeaders: Record<string, string> = {},
  includeCookies = true
): Record<string, string> {
...
function createCrawlerConfig(site: string, options: any = {}): any {
  const {
    timeoutMs = 10000,
    useGzip = true,
    useCookies = true,
