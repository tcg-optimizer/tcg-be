function parseYugiohRarity(title: string): string {
  const rarityPatterns = [
    ...
    {
      pattern:
        /(엑스트라\s*시크릿\s*레어|엑스트라시크릿레어|엑스트라\s*씨그릿|엑시크|extra\s*secret\s*rare)/i,
      rarity: '엑스트라 시크릿 레어',
    },
    {
...
function parseVanguardRarity(title: string): string {
  const rarityPatterns = [
...
function parseOnepieceRarity(title: string): string {
...
function normalizeRarity(
  rarity: string,
  options: { gameType?: string; cardCode?: string | null } = {}
): string {
...
// 게임 타입에 따라 레어도 파싱
function parseRarity(title: string, gameType = 'yugioh'): string {
  switch (gameType) {
    case 'vanguard':
