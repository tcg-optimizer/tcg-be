import { findGreedyOptimalPurchase } from './greedy-algorithm';
import { calculatePointsAmount } from './points-utils';
import { getSellerId, filterTopSellers, isNaverStore } from './card-utils';
import {
...
function findOptimalPurchaseCombination(cardsList: any, options: any = {}): any {
  try {
    ...
    if (options.pointsOptions) {
      Object.keys(options.pointsOptions).forEach(key => {
        if (Object.prototype.hasOwnProperty.call(mergedOptions.pointsOptions, key)) {
          (mergedOptions.pointsOptions as any)[key] = options.pointsOptions[key];
        }
      });
    }
    ...
    if (result.cardsOptimalPurchase) {
      ...
      Object.keys(result.cardsOptimalPurchase).forEach(seller => {
        ...
        if (sellerData.cards && Array.isArray(sellerData.cards)) {
          ...
          const filteredCards = sellerData.cards.filter((card: any) => {
          ...
          if (filteredCards.length !== originalCardCount) {
            ...
            if (filteredCards.length === 0) {
              ...
              const newProductCost = filteredCards.reduce((sum: any, card: any) => {
              ...
              if (seller.toLowerCase().includes('carddc') || seller.toLowerCase() === 'carddc') {
                ...
                result.cardsOptimalPurchase[seller].cards.forEach((card: any) => {
              ...
              result.cardsOptimalPurchase[seller].finalPrice =
                newProductCost + newShippingCost - newPointsEarned;
            }
          }
        }
      });
      ...
      if (excludedCardNames.size > 0) {
        ...
        for (const excludedCard of excludedCardsList) {
          ...
          const alternativeProducts = excludedCard.products.filter((product: any) => {
            const productId = product.id || (product.product && product.product.id);
            const productIdStr = productId ? String(productId) : '';
          ...
          alternativeProducts.sort((a: any, b: any) => a.price - b.price);
          ...
          if (sellerId.toLowerCase().includes('carddc') || sellerId.toLowerCase() === 'carddc') {
            ...
            let sellerPoints = 0;
            result.cardsOptimalPurchase[sellerId].cards.forEach((card: any) => {
              const productId = card.cardName;
      ...
      if (hasExcludedProducts && Object.keys(result.cardsOptimalPurchase).length > 0) {
        ...
        Object.entries(result.cardsOptimalPurchase).forEach(([_, sellerData]: [string, any]) => {
        ...
        result.totalCost = Object.values(result.cardsOptimalPurchase).reduce(
          (sum: any, sellerData: any) => sum + (sellerData.finalPrice || 0),
          0
        );
      }
    }
