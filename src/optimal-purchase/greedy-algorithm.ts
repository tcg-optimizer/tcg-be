import { getShippingInfo, calculateShippingFee, REGION_TYPES } from '../common/utils/shipping-info';
import { getSellerId, filterTopSellers } from './card-utils';
import { calculatePointsAmount } from './points-utils';
import {
...
function generateFreeShippingCombinations(
  ...
  pointsOptions: any,
  sellerShippingInfo: any
): any[] {
  const availableCards = sortedCards.filter(card =>
    card.products.some((p: any) => getSellerId(p.site) === seller)
  );

  const cardsWithPrice = availableCards.map(card => {
    const product = card.products.find((p: any) => getSellerId(p.site) === seller);
    const availablePrices = card.products.map((p: any) => p.price);
  ...
  // 재귀적으로 조합 생성 (백트래킹)
  function findCombinations(current: any[], startIdx: number, currentSum: number) {
  ...
  combinations.forEach(combo => {
    const totalPrice = combo.reduce((sum: any, item: any) => sum + item.totalPrice, 0);
    const totalPriceDifference = combo.reduce(
      (sum: any, item: any) => sum + item.priceDifference,
      0
    );
...
function findGreedyOptimalPurchase(cardsList: any[], options: any = {}): any {
  ...
  const sortingStrategies = [
    ...
    (cards: any[]) =>
      [...cards].sort((a, b) => {
        const aMinPrice = Math.min(...a.products.map((p: any) => p.price));
        const bMinPrice = Math.min(...b.products.map((p: any) => p.price));
    ...
    (cards: any[]) =>
      [...cards].sort((a, b) => {
        const aPrices = a.products.map((p: any) => p.price);
        const bPrices = b.products.map((p: any) => p.price);
  ...
  const reducedCardsList = filterTopSellers(cardsList, {
    maxSellersPerCard,
    excludedStores,
    excludedProductIds,
  });
  ...
  reducedCardsList.forEach(card => {
    card.products.forEach((product: any) => {
  ...
  const sellersList: any[] = Array.from(allSellers);

  const sellerShippingInfo: any = {};
  ...
  for (let strategyIndex = 0; strategyIndex < sortingStrategies.length; strategyIndex++) {
    ...
    const freeShippingCombinationsBySeller: any = {};
    ...
    const cardsOptimalPurchase: any[] = [];
    ...
    const efficientCombinations: any[] = [];
    ...
    for (const seller of freeShippingSellersSorted) {
      ...
      for (const combo of combinations) {
        ...
        for (const item of combo) {
          ...
          const otherProducts = cardInfo.products.filter(
            (p: any) => getSellerId(p.site) !== seller
          );
          ...
          if (otherProducts.length === 0) {
            ...
            const minPriceElsewhere =
              Math.min(...otherProducts.map((p: any) => p.price)) * item.quantity;
    ...
    for (const efficient of efficientCombinations) {
      ...
      // 이미 할당된 카드가 있으면 스킵
      const hasAssignedCard = combo.some((item: any) =>
          ...
          if (freeShippingCombinationsBySeller[sourceSellerName]) {
            const availableCombos = freeShippingCombinationsBySeller[sourceSellerName].filter(
              (combo: any) => {
                return combo.every(
                  (item: any) =>
                    !sourceSeller.cards.some((card: any) => card.cardName === item.card.cardName)
                );
              }
            );

            if (availableCombos.length > 0) {
              const potentialMoves: any[] = [];

              for (const combo of availableCombos) {
                const neededCards = combo.filter((item: any) =>
              ...
              if (potentialMoves.length > 0) {
                ...
                for (const seller of affectedSellers) {
                  ...
                  if (details && details.subtotal > 0) {
                    currentTotalCostBeforeMove +=
                      details.subtotal + details.shippingFee - (details.points || 0);
                  }
                }
                ...
                estimatedCostAfterMove +=
                  newSourceSubtotal + newSourceShippingFee - newSourcePoints;
                ...
                for (const seller of affectedSellers) {
                  ...
                  if (newSellerSubtotal > 0) {
                    ...
                    estimatedCostAfterMove +=
                      newSellerSubtotal + newSellerShippingFee - newSellerPoints;
                  }
                }
                ...
                for (const item of bestMove.neededCards) {
                  ...
                  if (existingCardIndex !== -1) {
                    ...
                    if (originalSellerName && purchaseDetails[originalSellerName]) {
                      ...
                      const cardIndexInOriginal = originalSeller.cards.findIndex((c: any) => {
        ...
        for (const card of [...sourceSeller.cards]) {
          ...
          for (const alt of alternatives) {
            ...
            if (
              ...
              const cardIndex = sourceSeller.cards.findIndex((c: any) => {
      ...
      for (const sellerToRemove of usedSellers) {
        ...
        for (const card of cardsToReassign) {
          ...
          const alternativeProducts = cardInfo.products.filter(
            (p: any) => getSellerId(p.site) !== sellerToRemove
          );
    ...
    if (totalCost < bestCost) {
      ...
      const finalPurchaseDetails: any = {};
      usedSellers.forEach(seller => {
        finalPurchaseDetails[seller] = purchaseDetails[seller];
      });

      const groupedCardsByStore: any = {};
      const cardImagesMap: any = {};
      ...
      cardsOptimalPurchase.forEach(card => {
        ...
        if (!cardImagesMap[imageKey]) {
          ...
          if (!selectedImage && cardInfo && cardInfo.products && cardInfo.products.length > 0) {
            const productWithImage = cardInfo.products.find((p: any) => p.image);
            if (productWithImage) {
      ...
      Object.keys(groupedCardsByStore).forEach(seller => {
        if (groupedCardsByStore[seller]) {
          ...
          const recalculatedProductCost = cards.reduce((sum: any, card: any) => {
          ...
          const recalculatedPoints = cards.reduce((sum: any, card: any) => {
            const cardPoints = calculatePointsAmount(
              seller,
              card.price,
              card.quantity,
          ...
          groupedCardsByStore[seller].finalPrice =
            recalculatedProductCost + recalculatedShippingCost - recalculatedPoints;
        }
      });

      // groupedCardsByStore에서 직접 집계하여 정확성 보장
      totalProductCost = Object.keys(groupedCardsByStore).reduce((sum, seller) => {
        const storeProductCost = groupedCardsByStore[seller]?.productCost || 0;
        return sum + storeProductCost;
      }, 0);

      totalShippingCost = Object.keys(groupedCardsByStore).reduce((sum, seller) => {
        const storeShippingCost = groupedCardsByStore[seller]?.shippingCost || 0;
        return sum + storeShippingCost;
      }, 0);

      totalPointsEarned = Object.keys(groupedCardsByStore).reduce((sum, seller) => {
...
function generateConsistentProductId(card: any): string {
...
function simpleStringHash(str: any): string {
  let hash = 0;
...
export { findGreedyOptimalPurchase };
