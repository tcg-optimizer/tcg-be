import { getSellerId } from './card-utils';
import { calculateShippingFee, REGION_TYPES } from '../common/utils/shipping-info';
import { calculatePointsAmount } from './points-utils';

function getCardKey(card: any): any {
...
function tryMoveCardsToReachThreshold(
  ...
  otherSellers.forEach(seller => {
    purchaseDetails[seller].cards.forEach((card: any) => {
      const cardInfo = cardsList.find((c: any) => getCardKey(c) === getCardKey(card));
      if (!cardInfo) return;

      const productInTargetSeller = cardInfo.products.find(
        (p: any) => getSellerId(p.site) === targetSeller
      );
  ...
  for (const candidate of candidateCards) {
    ...
    if (targetPrice >= gapToThreshold * 0.7 && targetPrice <= gapToThreshold * 1.5) {
      ...
      const movingCard = sourceSeller.cards.find(
        (c: any) => getCardKey(c) === getCardKey(candidate)
      );
      ...
      if (newTotalCost < currentTotalCost) {
        ...
        const actualNewTargetPoints =
          (purchaseDetails[targetSeller].points || 0) + targetCardPoints;
        ...
        purchaseDetails[targetSeller].total =
          newTargetSubtotal + newTargetShippingFee - actualNewTargetPoints;
...
function tryMultipleCardsMove(
  ...
  for (let numCards = 2; numCards <= maxCardsToMove; numCards++) {
    ...
    const generateCombinations = function (start: any, current: any, count: any) {
      ...
      for (let i = start; i < topCards.length; i++) {
        if (current.some((card: any) => card.seller === topCards[i].seller)) {
          continue;
        }
    ...
    for (const combination of combinations) {
      ...
      const originalDetails: any = {};
      combination.forEach((card: any) => {
      ...
      combination.forEach((card: any) => {
        ...
        const allCardsFromSeller = combination.filter((c: any) => c.seller === seller);
        const totalMovingPrice = allCardsFromSeller.reduce(
          (sum: any, c: any) => sum + c.currentPrice * c.quantity,
          0
        );
        ...
        movingCardsFromSeller.forEach((card: any) => {
          const cardInSeller = purchaseDetails[seller].cards.find(
            (c: any) => getCardKey(c) === getCardKey(card)
          );
          if (cardInSeller) {
      ...
      const involvedSellers = new Set(combination.map((card: any) => card.seller));
      involvedSellers.forEach((seller: any) => {
  ...
  if (
    ...
    bestTotalCost < Infinity &&
    bestTotalCost < bestCurrentStateCost
  ) {
    ...
    for (const card of bestCombination) {
      ...
      const movingCard = sourceSeller.cards.find((c: any) => getCardKey(c) === getCardKey(card));
      const movingCardPoints = movingCard ? movingCard.points || 0 : 0;
    ...
    Object.entries(sourceUpdates).forEach(([sellerName, update]: [string, any]) => {
      const seller = purchaseDetails[sellerName];

      update.cards.forEach((card: any) => {
        const cardIndex = seller.cards.findIndex((c: any) => getCardKey(c) === getCardKey(card));
        if (cardIndex !== -1) {
...
function trySellersConsolidation(
  ...
  for (const sourceSellerName of usedSellers) {
    ...
    for (const card of sourceSeller.cards) {
      ...
      cardInfo.products.forEach((product: any) => {
    ...
    Object.entries(movesByTargetSeller).forEach(([targetSeller, moves]: [string, any]) => {
      if (moves.length === sourceSeller.cards.length) {
        const totalPriceDiff = moves.reduce((sum: any, move: any) => sum + move.priceDifference, 0);
    ...
    if (bestTargetSeller && bestMovesCount === sourceSeller.cards.length) {
      ...
      if (newTotalCost < originalCost) {
        ...
        for (const move of moves) {
          const cardIndex = sourceSeller.cards.findIndex(
            (c: any) => getCardKey(c) === getCardKey(move)
          );
          if (cardIndex !== -1) {
            sourceSeller.cards.splice(cardIndex, 1);
          }
...
export { tryMoveCardsToReachThreshold, tryMultipleCardsMove, trySellersConsolidation };
