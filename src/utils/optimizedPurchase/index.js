  ...
  try {
    ...
    if (options.pointsOptions) {
      Object.keys(options.pointsOptions).forEach(key => {
        if (mergedOptions.pointsOptions.hasOwnProperty(key)) {
    ...
    const filteredCardsList = cardsList.map(card => {
      ...
      const filteredProducts = card.products.filter(product => {
        ...
        if (productIdStr) {
          for (const excludedId of excludedProductIds) {
            ...
            if (productIdStr === excludedIdStr) {
              ...
              console.log(`[INFO] 상품 ID "${productIdStr}"이 제외 목록에 있어 필터링됩니다. (카드: ${card.cardName || 'Unknown'})`);
              break;
            }
          }
        }
        ...
        if (shouldExcludeById && productIdStr) {
          console.error(`[DEBUG] optimizedPurchase: 상품 ID "${productIdStr}" 제외됨 (${card.cardName || 'Unknown'})`);
        }
    ...
    if (result.cardsOptimalPurchase) {
      ...
      Object.keys(result.cardsOptimalPurchase).forEach(seller => {
        ...
        if (sellerData.cards && Array.isArray(sellerData.cards)) {
          ...
          const excludedCards = sellerData.cards.filter(card => {
            if (card.product && card.product.id) {
              ...
              for (const excludedId of excludedProductIds) {
                const excludedIdStr = String(excludedId);
                if (productIdStr === excludedIdStr) {
                  console.error(`[심각한 오류] 최종 결과에서 제외된 상품 "${productIdStr}"가 발견되었습니다. (${card.cardName || '이름 없음'})`);
                  console.error(`  - 카드 정보: ${JSON.stringify({
          ...
          if (excludedCards.length > 0) {
            console.error(`[심각한 오류] ${seller}에서 ${excludedCards.length}개의 제외된 상품을 결과에서 제거합니다:`);
            excludedCards.forEach(card => {
              console.error(`  - ${card.cardName || '이름 없음'} (ID: ${card.product?.id || 'unknown'})`);
            });
            ...
            if (filteredCards.length === 0) {
              console.error(`[심각한 오류] ${seller}에 남은 카드가 없어 판매처를 결과에서 제거합니다.`);
      ...
      if (excludedCardNames.size > 0) {
        console.error(`[심각한 오류] 제외된 카드 ${excludedCardNames.size}개에 대해 대체 상품을 찾아 최적 조합 재계산을 시도합니다.`);
        ...
        for (const excludedCard of excludedCardsList) {
          ...
          if (alternativeProducts.length === 0) {
            console.error(`[심각한 오류] "${excludedCard.cardName}" 카드의 대체 상품을 찾을 수 없습니다.`);
            continue;
          }
          ...
          console.error(`[심각한 오류] "${excludedCard.cardName}" 카드의 최저가 대체 상품 찾음: ${sellerId}의 ${bestAlternative.price}원 상품`);
          ...
          result.cardsOptimalPurchase[sellerId].shippingCost = 
            (result.cardsOptimalPurchase[sellerId].productCost >= shippingInfo.freeShippingThreshold && 
             shippingInfo.freeShippingThreshold !== Infinity) ? 0 : shippingInfo.shippingFee;
