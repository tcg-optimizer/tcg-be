...
async function crawlTCGShopVanguard(
  cardName: string,
  ...
  try {
    ...
    productTables.each((index, element) => {
      ...
      let extractedCardCode: string | null = null;
...
async function crawlTCGShop(
  ...
  try {
    ...
    productCells.each((index, element) => {
      ...
      const discountRow = priceRow.next();
      let price = 0;
...
export async function searchAndSaveTCGShopPrices(
  cardName: string,
  gameType: string = GAME_TYPES.YUGIOH
): Promise<any> {
  try {
...
export {
  crawlTCGShopWithRateLimit as crawlTCGShop,
  crawlTCGShopVanguardWithRateLimit as crawlTCGShopVanguard,
};
