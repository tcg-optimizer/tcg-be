...
// 유희왕 언어 파싱
function parseYugiohLanguage(title: string): string {
...
// 뱅가드 언어 파싱
function parseVanguardLanguage(title: string): string {
  // 상품명에 언어가 명시된 경우
  if (/(한글판|한판)/i.test(title)) {
    return '한글판';
  }
...
function parseOnepieceLanguage(title: string): string {
...
// 게임 타입에 따라 언어 파싱
export function parseLanguage(title: string, gameType: string = 'yugioh'): string {
  switch (gameType) {
    case 'vanguard':
...
// 유희왕 카드 코드 추출
function extractYugiohCardCode(title: string): string | null {
...
function extractVanguardCardCode(title: string): string | null {
  ...
  const patterns = [
    ...
    // 패턴 13-1: (DZ-BT09-KREX01) - 기존 패턴 (숫자로 시작)
    {
      ...
      handler: (m: RegExpMatchArray) => `${m[1]}-${m[2]}${m[3]}/${m[5]}${m[4] ? ` ${m[4]}` : ''}`,
    },
    ...
    {
      regex:
        /\b([A-Z]{1,2})-([A-Z]{2,4})(\d{2})\s*\/\s*(\d{3})\s*\/\s*[A-Z]+\s*\/\s*(한글판|일본판|영문판)/i,
    ...
    {
      regex:
        /\/\s*([A-Z]{1,2})-([A-Z]{2,4})(\d{2})\s*\/\s*(\d{3})\s*\/\s*[A-Z]+\s*\/\s*(한글판|일본판|영문판)/i,
      handler: (m: RegExpMatchArray) => {
        const lang = m[5] === '한글판' ? 'KR' : m[5] === '일본판' ? 'JP' : 'EN';
        return `${m[1]}-${m[2]}${m[3]}/${m[4]} ${lang}`;
      },
...
function extractOnepieceCardCode(title: string): string | null {
  ...
  for (const pattern of patterns) {
    ...
    if (match && match[1]) {
      ...
      code = code.replace(/((?:-\d{3}|-(?:KR|JP|EN)-?\d{3}))(PP|SEC|SR|SP|TR|UC|L|R|C|P)$/i, '$1');
...
// 게임 타입에 따라 카드 코드 추출
export function extractCardCode(title: string, gameType: string = 'yugioh'): string | null {
  switch (gameType) {
    case 'vanguard':
